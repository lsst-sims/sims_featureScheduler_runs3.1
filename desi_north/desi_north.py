...
class StepLineMax(BasePixelEvolution):
    """
    Parameters
    ----------
    period : float (365.25)
    ...
    def __call__(self, mjd_in, phase, max_t=365.*2):
        
        result = self._orig(mjd_in, phase)
        max_result = self._orig(max_t, phase)

        over = np.where(result > max_result)[0]
        result[over] = max_result[over]
        return result
...
def make_rolling_footprints(
    ...
    # ok, now we have all the usual set up, let's also make a footprint
    # object for the north that only runs for 2 years
    step_func_2 = StepLineMax()
    fp_north = Footprint(mjd_start, sun_ra_start=sun_ra_start, nside=nside, step_func=step_func_2)
    for key in fp_north_hp:
        fp_north.set_footprint(key, fp_north_hp[key])

    result = Footprints([fp_north] + [fp_non_wfd] + rolling_footprints)
    return result


class DesiFootprint(EuclidOverlapFootprint):
    def add_desi(self, filter_ratios, label="desi north", lower_dec=-20,
                 upper_dec=20, gp=20):

        # Set the low extinction area
        indx = np.where((np.abs(self.gal_lat) > gp) & 
                            (self.dec > lower_dec) & 
                            (self.dec < upper_dec) &
                            (self.pix_labels == ""))
    ...
    def return_maps(
        ...
        desi_ratios={"u": 0.32, "g": 0.4, "r": 1.0, "i": 1.0, "z": 0.9, "y": 0.9},
    ):
        """
        Parameters:
        various_ratios : `dict`
