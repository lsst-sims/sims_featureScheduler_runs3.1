...
def run_sched(surveys, survey_length=365.25, nside=32, fileroot='baseline_', verbose=False,
              extra_info=None, illum_limit=40., mjd_start=60676., cloud_offset_year=0):
    years = np.round(survey_length/365.25)
    scheduler = CoreScheduler(surveys, nside=nside)
    n_visit_limit = None
    fs = FilterSchedUzy(illum_limit=illum_limit)
    observatory = ModelObservatory(nside=nside, mjd_start=mjd_start, cloud_db='clouds.db', cloud_offset_year=cloud_offset_year)
...
def main(args):
    ...
    if dbroot is None:
        ...
        fileroot = dbroot + '_'
    file_end = 'cloudso%i' % cloud_offset_year + 'v3.1_'
    ...
    observatory, scheduler, observations = run_sched(surveys, survey_length=survey_length, verbose=verbose,
              ...
              nside=nside, illum_limit=illum_limit, mjd_start=mjd_start, cloud_offset_year=cloud_offset_year)
