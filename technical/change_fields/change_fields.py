...
from rubin_sim.scheduler.surveys import (GreedySurvey, BlobSurvey, ScriptedSurvey, LongGapSurvey)
...
from rubin_sim.utils import _hpid2_ra_dec, angular_separation, _ra_dec_from_xyz
import rubin_sim
...
def blob_for_long(nside, nexp=2, exptime=30., filter1s=['g'],
    ...
    for filtername, filtername2 in zip(filter1s, filter2s):
        ...
        surveys.append(BlobSurvey(basis_functions, weights, filtername1=filtername, filtername2=filtername2,
                                  ...
                                  nexp=nexp, detailers=detailer_list, fields=fields,
                                  **BlobSurvey_params))
...
def gen_long_gaps_survey(footprints, nside=32, night_pattern=[True, True],
                         ...
                         time_after_twi=120, nights_delayed=-1, fields=None):
    """
    Paramterers
    -----------
    HA_min(_max) : float
    ...
    for filtername1, filtername2 in zip(f1, f2):
        blob = blob_for_long(footprints=footprints, nside=nside, filter1s=[filtername1],
                             ...
                             HA_min=HA_min, HA_max=HA_max, nights_delayed=nights_delayed, fields=fields)
...
def gen_GreedySurveys(nside=32, nexp=2, exptime=30., filters=['r', 'i', 'z', 'y'],
                      ...
                      stayfilter_weight=3., repeat_weight=-1., footprints=None, fields=None):
    """
    ...
    for filtername in filters:
        ...
        surveys.append(GreedySurvey(basis_functions, weights, exptime=exptime, filtername=filtername,
                                    ...
                                    detailers=detailer_list, fields=fields,
                                    **greed_survey_params))
...
def generate_blobs(nside, nexp=2, exptime=30., filter1s=['u', 'u', 'g', 'r', 'i', 'z', 'y'],
                   ...
                   mjd_start=1, repeat_weight=-20, fields=None):
    """
    Generate surveys that take observations in blobs.
...
def generate_twi_blobs(nside, nexp=2, exptime=30., filter1s=['r', 'i', 'z', 'y'],
                       ...
                       night_pattern=None, fields=None):
    """
    Generate surveys that take observations in blobs.
    ...
    for filtername, filtername2 in zip(filter1s, filter2s):
        ...
        surveys.append(BlobSurvey(basis_functions, weights, filtername1=filtername, filtername2=filtername2,
                                  ...
                                  ideal_pair_time=pair_time,
                                  survey_note=survey_name, ignore_obs=ignore_obs,
                                  nexp=nexp, detailers=detailer_list, fields=None,
                                  **BlobSurvey_params))
...
def generate_twilight_neo(nside, night_pattern=None, nexp=1, exptime=15,
                          ...
                          filters='riz', n_repeat=3, sun_alt_limit=-14.8, fields=None):
    ...
    for filtername in filters:
        ...
        surveys.append(BlobSurvey(basis_functions, weights, filtername1=filtername, filtername2=None,
                                  ...
                                  area_required=area_required, fields=fields))
    return surveys


def run_sched(surveys, survey_length=365.25, nside=32, fileroot='baseline_', verbose=False,
              extra_info=None, illum_limit=40., mjd_start=60676.):
...
def main(args):
    ...
    field_indx = args.field_indx

    # Files with field positions
    field_files = glob.glob('icover*.txt')
    field_files.sort()

    field_file = field_files[field_indx]
    points = np.genfromtxt(field_file)
    x = points[0::3]
    y = points[1::3]
    z = points[2::3]

    ra, dec = _ra_dec_from_xyz(x, y, z)
    fields = np.zeros(ra.size, dtype=list(zip(['RA', 'dec'], [float, float])))
    fields['RA'] = ra
    fields['dec'] = dec
    ...
    camera_ddf_rot_limit = 75.  # degrees
    ...
    if dbroot is None:
        ...
        fileroot = dbroot + '_'
    file_end = 'fi_%i_v3.1_' % field_indx
    ...
    greedy = gen_GreedySurveys(nside, nexp=nexp, footprints=footprints, fields=fields)
    neo = generate_twilight_neo(nside, night_pattern=neo_night_pattern,
                                filters=neo_filters, n_repeat=neo_repeat,
                                footprint_mask=footprint_mask, fields=fields)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--field_indx", type=int, default=0)
